...
public class ManagePanel extends VerticalPanel implements Initializable {
    ...
    private static final String[] HEADERS = {"排名", "投资部门", "初始资金", "可用资金", "股票市值", "总市值", "盈亏总额", "盈亏比例", "股票持仓比例", " ", " "};
    ...
    private void insertRankTableRow(int row, final AccountInfo info) {
        ...
        if (info.getTotalValue() != 0) {
            rankGrid.setWidget(row, 8, new Label(fmt.format(100.0 * info.getStockValue() / info.getTotalValue()) + "%"));
        } else {
